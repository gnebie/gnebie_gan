...
class SimpleGan(AbstractGanModel):
    ...
    def conbinate_model(self, discriminators, generators):
    model_list = list()
    ...
    def create_generator(self, latent_dim, n_blocks, in_dim=4):
        ...
        self._generator = model_list
        return model_list
        
    def create_discriminator(self, n_blocks, input_shape=(4,4,3)):
        # weight initialization
        init = RandomNormal(stddev=0.02)
        # weight constraint
        const = max_norm(1.0)
        ...
        for i in range(1, n_blocks):
            ...
            # store model
            model_list.append(models)
        self._discriminator = model_list
        return model_list
    ...
    def train_epochs(self, g_model, d_model, gan_model, dataset, n_epochs, n_batch, fadein=False):
    ...
    def run_train_step(self, images, step):
        g_models = self.generator
        d_models = self.discriminator
        gan_models = self.combine_model
        dataset = self.dataset
        latent_dim = 100
        e_norm = [5, 8, 8, 10, 10, 10]
        e_fadein = [5, 8, 8, 10, 10, 10]
